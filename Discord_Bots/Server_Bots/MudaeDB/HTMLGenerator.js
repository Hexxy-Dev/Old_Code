...
async function Start(){
    ...
    globalPath = './MudaeDB/lists/Global/Global';
    separator = '';
...
async function GenerateHTML(UserList){
    ...
    if(fs.existsSync('./MudaeDB'+userPath+'.json')){
        for(j=0;j<=5;j++){
...
async function createHtml(jsonInfo,author,path,SortType){
    if(fs.existsSync('./MudaeDB'+path+`${SortType}`+'.html')){
        fs.unlinkSync('./MudaeDB'+path+`${SortType}`+'.html');
    }
    ...
    for(i=0;i<Object.keys(jsonInfo).length;i++){
        ...
        await fsPromises.appendFile(`./MudaeDB/lists/${author}/${author}${SortType}.html`,`\n<div class="CharacterInfo" id="${jsonInfo[i].Name}">` + info + '\n</div>\n');
    }
    return true;
}
...
async function createGlobalJSON(userList,separator,globalPath){
    ...
    if(fs.existsSync('./MudaeDB'+userPath)){
        ...
        for(k=0;k<Object.keys(jsonUserInfo).length;k++){
            ...
            fs.appendFileSync(`${globalPath}.json`,jsonWriteInfo, function (err) {
                if (err) throw err;
