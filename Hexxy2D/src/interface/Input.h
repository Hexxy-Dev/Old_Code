...
namespace Input
{
	...
	{
		pointers[keysOrder[key] * 3 + actionsOrder[action]] = function;
	}

	void OnScroll(void (*function)(double, double))
	{
		scrollPointer = function;
	}

	bool IsPressed(const char* key)
	{
		return pressedKeys[keysOrder[key]];
	}
}

static void key_callback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	pointers[keys[key] * 3 + actions[action]]();
...
{
	pointers[keys[button] * 3 + actions[action]]();
...
{
	Input::XMousePos = xpos / QuadSize;
	Input::YMousePos = ypos / QuadSize;
}
