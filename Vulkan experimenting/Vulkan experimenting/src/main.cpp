...
private:
	...
	void listExtensions()
	{
		...
		uint32_t availableRequiredExtensions = 0;
		for (int i = 0; i < glfwExtensionCount; i++)
		{
			for (const auto& extension : extensions)
			{
		...
		if (availableRequiredExtensions != glfwExtensionCount)
		{
			throw std::runtime_error("Required extensions for glfw are missing!");
		}
	}
	...
	void createGraphicsPipeline()
	{
		...
		rasterizer.polygonMode = VK_POLYGON_MODE_FILL; // this is where wireframe can be set | requires gpu feature
		...
		*/
		...
		VkDynamicState dynamicStates[] = { // not used
