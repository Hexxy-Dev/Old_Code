...
enum Difficulty {
	...
	Normal, // block 2 in a row
	Hard, // block 2, start first
	Impossible //bot always starts first, perfect play
};

//Settings
bool piece = 1; //0 to start with O   1 to start with X
Difficulty difficulty = Difficulty::Random;

///////////

bool botStartFirst = false;
bool botTurn = false;
...
int main(){
	...
	if (difficulty == Difficulty::Random)
		difficulty = diffs[rand() % 4];

	if (difficulty == Difficulty::Impossible || difficulty == Difficulty::Hard)
		botStartFirst = true;
